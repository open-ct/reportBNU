...
$(document).ready(function(){
	...
	$("#dwDialogSaveLogic").click(function(){
		...
		$.each(dwQuLogicTrs,function(){
			...
			var skQuId=$(this).find(".logicQuSel:eq(0)").val();
			var skQuIdEnd=$(this).find(".logicQuSel:eq(1)").val();
			...
			if(skQuId!="" && cgQuItemId!=""){
				...
				if(quLogicItem[0]){
					...
					var oldSkQuIdEnd=quLogicItem.find("input[name='skQuIdEnd']").val();
					var oldCgQuItemId=quLogicItem.find("input[name='cgQuItemId']").val();
					var oldLogicType=quLogicItem.find("input[name='logicType']").val();
					
					if(oldSkQuId!=skQuId || cgQuItemId!=oldCgQuItemId || oldLogicType!=logicType || oldSkQuIdEnd!=skQuIdEnd ){
						...
						quLogicItem.find("input[name='skQuIdEnd']").val(skQuIdEnd);
					}
					//如果是评分题
					if(quType==="SCORE"){
						//geLe scoreNum
...
//						quLogicItem.find("input[name='logicEvent']").val(logicEvent);
	...
	$("#dwDialogQuSetSave").click(function(){
		if(dwDialogObj!=null){
			...
			if(quType=="CHECKBOX"){
				var maxNum = $("#dwCommonDialog input[name='maxNum']").val();
				quItemBody.find("input[name='paramInt01']").val(maxNum);
			}
		...
		saveQus(fristQuItemBody,function(){
			...
			if(quLogicItems[0]){
				$.each(quLogicItems,function(){
					...
					var skQuIdEnd=$(this).find("input[name='skQuIdEnd']").val();
					...
					addQuDialogLogicTr(false,function(){
						...
						if(logicType==="3"){
							lastTr.find(".logicQuSel").show();
							lastTr.find(".logicQuOptionSel").val(cgQuItemId);
							lastTr.find(".logicType").val(logicType);
							lastTr.find(".logicQuSel:eq(0)").val(skQuId);
							lastTr.find(".logicQuSel:eq(1)").val(skQuIdEnd);
						}else{
							lastTr.find(".logicQuSel:eq(0)").show();
							lastTr.find(".logicQuOptionSel").val(cgQuItemId);
...
function showDialog(thDialogObj){
	...
	if(thObjClass.indexOf("addMoreOption")>=0){
		...
		if(quType==="CHECKBOX"){
			...
			if(paramInt01[0]){
				$("#dwCommonDialog input[name='maxNum']").val(paramInt01.val());
			}
		}
		//单选，多选 才启用选项随机排列
		if(quType==="RADIO" || quType==="CHECKBOX"){
	...
	if(saveTag==0){
		...
		$.each(quLogicItems,function(i){
			...
			if(logicSaveTag==0){
				...
				data+="&skQuIdEnd_"+itemIndex+"="+skQuIdEnd;
			}
		});
		$.ajax({
			url:url,
...
function bindDialogRemoveLogic(){
	...
	$("#dwQuLogicTable .logicType").unbind();
	$("#dwQuLogicTable .logicType").change(function(){
		var thVal = $(this).val();
		var thTr = $(this).parents("tr");
		if(thVal==="3" ){
			thTr.find(".logicQuSel").show();
		}else{
			thTr.find(".logicQuSel:eq(1)").hide();
		}
	});
}
