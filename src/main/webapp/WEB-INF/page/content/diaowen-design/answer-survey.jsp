...
function ready(){
	...
	$.each(quLogics,function(){
        ...
        var skQuIdEnd=loginItem.find(".skQuIdEnd").val();
		if(logicType==="2"){
			//逻辑类型为“显示”2  则初始化为隐藏
            ...
            hidQuItemBody.find(".answerTag").attr("disabled",true);
            //skQuIdEnd;
            hide3Qus(hidQuItemBody.next(),skQuIdEnd,logicId);
        }
	});
	
	if(localStorage.getItem("surveyid")=="${survey.id}" && localStorage.getItem("userid")==userId){
		$("#dwSurveyQuContent").html(localStorage.getItem("html"));
		$("#resultProgressRoot").html(localStorage.getItem("bar"));
		$("input[type='radio']").each(function(i,item){
			if(localStorage.getItem("radio"+i)=="true") $(item).prop("checked",true);
			else $(item).prop("checked",false);
		});
		$("input[type='checkbox']").each(function(i,item){
			if(localStorage.getItem("checkbox"+i)=="true") $(item).prop("checked",true);
			else $(item).prop("checked",false);
		});
		$(".fillblankInput").each(function(i,item){
			$(item).val(localStorage.getItem("fillblank"+i));
		});
		$(".dwMFillblankInput").each(function(i,item){
			$(item).val(localStorage.getItem("mfillblank"+i));
		});
		$(".dwChenMFillblankInput").each(function(i,item){
			$(item).val(localStorage.getItem("cfillblank"+i));
		});
		$(".quChenScoreSelect").each(function(i,item){
			$(item).val(localStorage.getItem("cscore"+i));
		});
		intDiff = localStorage.getItem("diff");
	}
	...
	function validateCheck(quItemBody,isSubForm){
		if(quItemBody.is(":visible")){
			...
			if(quType==="FILLBLANK"){
				if(quItemBody.find(".fillblankInput").val().length>200) lengthStatus=false;
				...
				$.each(quScoreOptionTrs,function(){
					...
					if(scoreNumInput.val().length>200){
						lengthStatus=false;
						return false;
					}
				});
			...
			if(!lengthStatus){
				if(isSubForm && !quItemBody.find(".errorItem")[0]){
					var errorHtml="<div class=\"errorItem\"><label for=\"\" class=\"error\">请检查题目答案，填空题长度不能超过200个汉字或字符！</label></div>";
					quItemBody.find(".quCoItem").append(errorHtml);
				}
				return false;
			}
			else quItemBody.find(".errorItem").remove();
...
//				thObj.find("input[type='checkbox']").prop("checked",true);
				...
				var paramInt01 = quItemBody.find("input[name='paramInt01']").val();
				var checkboxCheckeds = quItemBody.find("input[type='checkbox']:checked");
				//alert(checkboxCheckeds.size());
				if(paramInt01>0){
					if(checkboxCheckeds.size() < paramInt01){
						dwQuInputLabel.addClass("checked");
						thObj.find("input[type='checkbox']").prop("checked",true);
					}else{
						alert("最多选"+paramInt01+"项！");
					}
				}else{
					dwQuInputLabel.addClass("checked");
					thObj.find("input[type='checkbox']").prop("checked",true);
    ...
    function runlogic(thFormElementObj){
    	...
    	if(quLogicItems[0]){
			...
			quInputCase=quItemBody.find(".quInputCase");
			quId=quInputCase.find(".quId").val();
			quType=quInputCase.find(".quType").val();
							...
							if(logicType==="1"){
                                ...
                                if(logicStatus){
									...
									*/
									var hidQuItemBody=$(".quId[value='"+skQuId+"']").parents(".li_surveyQuItemBody");
                                    hidQuItemBody.show();
                                    hidQuItemBody.find(".answerTag").attr("disabled",false);
                                    //skQuIdEnd;
                                    show3Qus(hidQuItemBody.next(),skQuIdEnd,logicId);
								...
								if(logicStatus){
									...
									var hidQuItemBodys=$(".hidFor"+logicId);
									$(".hidFor"+logicId).removeClass("hidFor"+logicId);
									var hidQuItemBodys=$(".quId[value='"+skQuId+"']").parents(".li_surveyQuItemBody");
    ...
    function clearans(quItemBody){
		...
		if(quType==="RADIO"){
			...
			quItemBody.find(".dwQuInputLabel").removeClass("checked");
			quItemBody.find("input[type='checkbox']").prop("checked",false);
		}else if(quType==="FILLBLANK"){
			quItemBody.find(".fillblankInput").val("");
		}else if(quType==="ORDERQU"){
			var quOrdersTrs=quItemBody.find(".quOrderByTable label");
			$.each(quOrdersTrs,function(i,orderTr){
				$(orderTr).attr("class", "editAble quCoOptionEdit");
				var quOrderLeft=quItemBody.find(".quOrderByLeftUl li");
				$.each(quOrderLeft,function(i,left){
					if(!$(left).find("label")[0]){
						$(left).append($(orderTr));
						return false;
					}
				});
			...
			$.each(quScoreOptionTrs,function(){
				var scoreNumInput=$(this).find(".scoreNumInput");
				scoreNumInput.val("");
				$(this).find(".scoreNumTable tr td").css({"background":"white"});
				$(this).find(".scoreNumText").html("&nbsp;分");
			});
		}else if(quType==="MULTIFILLBLANK"){
			var quScoreOptionTrs=quItemBody.find(".mFillblankTableTr");
			$.each(quScoreOptionTrs,function(){
			...
			var quScoreOptionTrs=quItemBody.find(".dwQuCoChenRowTr");
			$.each(quScoreOptionTrs,function(){
				$(this).find(".dwQuInputLabel").removeClass("checked");
	...
	function hide3Qus(nextHidQuItemBody,skQuIdEnd,logicId){
        if(nextHidQuItemBody[0]){
            //alert(logicId);
            var nextQuType=nextHidQuItemBody.find(".quType").val();
            var nextQuId=nextHidQuItemBody.find(".quId").val();
            var nextAnswerTag=nextHidQuItemBody.find(".answerTag");

			clearans(nextHidQuItemBody);
            nextHidQuItemBody.hide();
            nextHidQuItemBody.find(".answerTag").attr("disabled",true);
            if(nextQuId===skQuIdEnd){
                //停止查找
//                alert("skQuIdEnd:"+skQuIdEnd);
            }else{
                hide3Qus(nextHidQuItemBody.next(),skQuIdEnd,logicId);
            }
        }
    }
    
    function show3Qus(nextHidQuItemBody,skQuIdEnd,logicId){
        if(nextHidQuItemBody[0]){
            //alert(logicId);
            var nextQuType=nextHidQuItemBody.find(".quType").val();
            var nextQuId=nextHidQuItemBody.find(".quId").val();
            ...
            nextHidQuItemBody.show();
            nextHidQuItemBody.find(".answerTag").attr("disabled",false);
            if(nextQuId===skQuIdEnd){
                //停止查找
//                alert("skQuIdEnd:"+skQuIdEnd);
            }else{
                show3Qus(nextHidQuItemBody.next(),skQuIdEnd,logicId);
            }
        }
    }
	...
	*/
	...
	function localsave(){
		...
		localStorage.setItem("html",$("#dwSurveyQuContent").html());
		localStorage.setItem("diff",intDiff);
		localStorage.setItem("bar",$("#resultProgressRoot").html());
		$("input[type='radio']").each(function(i,item){
			localStorage.setItem("radio"+i,$(item).prop("checked"));
		});
		$("input[type='checkbox']").each(function(i,item){
			localStorage.setItem("checkbox"+i,$(item).prop("checked"));
		});
		$(".fillblankInput").each(function(i,item){
			localStorage.setItem("fillblank"+i,$(item).val());
		});
		$(".dwMFillblankInput").each(function(i,item){
			localStorage.setItem("mfillblank"+i,$(item).val());
		});
		$(".dwChenMFillblankInput").each(function(i,item){
			localStorage.setItem("cfillblank"+i,$(item).val());
		});
		$(".quChenScoreSelect").each(function(i,item){
			localStorage.setItem("cscore"+i,$(item).val());
		});
	...
	<%--<div id="header" >
		<div id="headerCenter"  class="bodyCenter">
			<div class="header_Item header_logo">
				<img alt="调问网" src="${ctx }/images/logo/logo-xt.png" height="46" >
			</div>
		</div>
...
<input type="hidden" id="pcMac" name="pcMac" value="0">
<div id="dw_body" style="padding-top:0px;">
	<div id="dw_body_content">
		<div id="dwSurveyHeader" style="padding-top: 0px;margin-top: 13px;">
			<div id="dwSurveyLogo" style="display: block;background: rgb(83, 136, 99);">
				<%--<img src="${ctx }/images/logo/logo-xt-ne.png" height="56" style="border: none;"/>--%>
				<img src="${ctx }/images/logo/logo-xt-2.png" height="56" style="border: none;"/>
				<div id="dwSurveyLogoTimer" style="float: right;color: #d0cece;border: 1px dashed #d0cece;padding: 0px 13px;">
					<div>答卷倒计时&nbsp;<span id="minute_show" class="timeSpan">＊＊</span>&nbsp;分 <span id="second_show" class="timeSpan">＊＊</span>&nbsp;秒</div>
					<div style="font-size: 14px;">倒计时结束，将自动提交，请尽快作答。</div>
				</div>
			</div>
		...
		<div id="dwSurveyQuContent" style="min-height: 300px;">
			...
			<input type="hidden" id="stamp3" name="stamp3" value="0" >
							...
							<div class="quInputCase" style="display: none;">
								...
								<c:forEach items="${en.questionLogics }" var="quLogicEn" varStatus="logicSts">
									<div class="quLogicItem quLogicItem_${logicSts.count }">
                                        ...
                                        <input type="hidden" class="skQuIdEnd" value="${quLogicEn.skQuIdEnd }"/>
									</div>
								</c:forEach>
								</div>
							</div>
							...
							<div class="surveyQuItem">
								<div class="surveyQuItemContent">
									...
									<div class="quCoItem">
										...
										<table class="quCoChenTable" style="table-layout:fixed;">
												<tr><td width="200"></td>
												...
												<tr class="dwQuCoChenRowTr">
														<td class="quChenRowTd" style="text-align: left;padding-left: 12px;"><label class="editAble quCoOptionEdit">${rowItem.optionName }</label>
														<input type="hidden" class="answerTag" value="0" >
														</td>
														<c:forEach items="${en.columns }" var="columnItem">
														<td>
								...
								<div class="surveyQuItemContent">
									...
									<div class="quCoItem">
										...
										<table class="quCoChenTable" >
												...
												<c:forEach items="${en.rows }" var="rowItem">
												<tr class="dwQuCoChenRowTr"><td class="quChenRowTd"  style="text-align: left;padding-left: 12px;"><label class="editAble quCoOptionEdit">${rowItem.optionName }</label>
				...
				<li class="li_surveyQuItemBody surveyQu_${pageNo }"  style="${pageNo gt 1 ?'display: none':''}">
					<div class="surveyQuItemBody">
							<div class="surveyQuItem">
								...
								<div class="surveyQuItemContent" style="padding-top: 12px;height: 30px;min-height: 30px;text-align: center;">
...
<div class="footer-copyright footer-pb" style="color: gray;padding-bottom: 5px;">
		...
		<%--	<div style="margin-top: 10px;">Copyright © 2017 版权所有 中国基础教育质量监测协同创新中心</div>
			<div style="line-height: 26px;margin-top: 10px;padding-bottom: 10px;">电话：000-11101010&nbsp;&nbsp;QQ：893232222</div>--%>
</div>
...
var intDiff = parseInt(3600*24);//倒计时总秒数量
function timer(){
	window.setInterval(function(){
		var day=0,
				hour=0,
		...
		if(intDiff > 0){
			...
			second = Math.floor(intDiff) - (minute * 60);//(day * 24 * 60 * 60) - (hour * 60 * 60) - (minute * 60);
		}
...
$(function(){
	...
	$("#dwSurveyLogoTimer").hide();
	if(${limitTime} != -1){
	//if(surveyType==="11" || surveyType==="21"){
		$("#dwSurveyLogoTimer").show();
		intDiff = 60 * ${limitTime};
		timer();
	}
});
